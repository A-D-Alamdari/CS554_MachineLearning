...
#
#
# def plot_comparative_metrics(all_results, metric_type='loss'):
#     """
#     Plots training and test metrics (loss or accuracy) for all models.
#
#     Parameters:
#     - all_results: dict of model_name -> dict with keys: train_loss, test_loss, train_acc, test_acc
...
#             plt.plot(metrics['test_loss'], linestyle='--', label=f'{model_name} Test')
#             ylabel = 'Loss'
#         elif metric_type == 'accuracy':
#             plt.plot(metrics['train_acc'], label=f'{model_name} Train')
#             plt.plot(metrics['test_acc'], linestyle='--', label=f'{model_name} Test')
#             ylabel = 'Accuracy'
#
#     plt.xlabel('Epoch')
#     plt.ylabel(ylabel)
#     plt.title(f'Comparison of {metric_type.capitalize()} Across Models')
#     plt.legend()
#     plt.grid(True)
#     plt.savefig(f'comparison_{metric_type}.png')
#     plt.close()
...
def plot_metrics(train_losses, test_losses, train_accs, test_accs, title_prefix):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(RESULTS_DIR, f'{title_prefix.lower()}_loss.png'))
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(RESULTS_DIR, f'{title_prefix.lower()}_accuracy.png'))
    plt.close()
...
def plot_confusion_matrix(model, data_loader, device, title_prefix):
    ...
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.savefig(os.path.join(RESULTS_DIR, f'{title_prefix.lower()}_confusion_matrix.png'))
    plt.close()
...
def plot_misclassified_examples(model, data_loader, device, class_names, title_prefix, max_examples=9):
    ...
    plt.suptitle(f"{title_prefix} Misclassified Examples")
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f'{title_prefix.lower()}_misclassified.png'))
    plt.close()
...
def plot_comparative_metrics(all_results, metric_type='loss'):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(RESULTS_DIR, f'comparison_{metric_type}.png'))
    plt.close()